...
- **Weakest** = the topic with the lowest `rate`. Tiebreaker: highest `attempts_topic` (the one we know most confidently is weak). Second tiebreaker: lowest topic number (1–6).
...
When a question genuinely tests two topics (per `quiz-rules-shared.md` Topic-Attribution Rule), write the SAME concept row in BOTH topic concept files. Mark the concept name with a trailing `(↔ {other-topic})`. Stack-flavored examples in the per-stack addendum.
...
## Cross-Stack-Question Attribution

For questions drawn from `cross-stack-rosetta.md` (per `quiz-rules-shared.md` Cross-Stack Question Rule), attribute the concept to the **primary-stack topic** for the active tutor:

- `cuda-tutor` asking "What's the Triton equivalent of CUTLASS Stages?" → concept tracked in `concepts/cutlass.md` (the CUDA-side primary topic).
- `triton-tutor` asking "What's the CUTLASS equivalent of `num_stages`?" → concept tracked in `concepts/tiling-autotuning.md` (the Triton-side primary topic).

Cross-stack questions do NOT dual-attribute by default — each tutor maintains its own vault. The Rosetta itself is the cross-stack shared surface.
...
| Concept tested 10 times all wrong | `Attempts=10`, `Correct=0`, rate 0 %, badge for topic 🟥 (if it's the only concept). |
...
| Concept file does not yet exist | Create it from the template in the parent skill's `SKILL.md` before adding the first row. |
| `attempts_topic == 0` (division by zero) | Skip the rate formula entirely; emit badge ⬜ Unmeasured and rate `-`. Same applies to `attempts_total == 0`. |
| Dashboard does not yet exist | Create from the template in the parent skill's `SKILL.md`; initialize all 6 rows with `0/0/-/⬜`. |
...
Whenever you finish the file-update phase of a quiz workflow, the dashboard's `Total` row must satisfy:
