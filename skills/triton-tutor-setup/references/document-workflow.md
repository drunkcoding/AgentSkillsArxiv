# Document Mode — Triton-Aware Workflow

> Transform PDFs (Triton paper, FlashAttention papers, OpenAI/Meta/PyTorch blog posts as PDFs, GTC slide decks on Triton), text, web pages, and Jupyter notebooks into study notes.
> All scanning and output MUST stay within CWD.

## Phase D1: Source Discovery & Extraction

1. **Auto-scan CWD** for `**/*.pdf`, `**/*.txt`, `**/*.md` (excluding `README.md` and `StudyVault/`), `**/*.html`, `**/*.epub`, `**/*.ipynb`. Exclude `node_modules/`, `.git/`, `dist/`, `build/`.
2. **Extract text (MANDATORY tools)**:
   ...
   - **Jupyter notebooks** → `jupyter nbconvert --to markdown notebook.ipynb` (preserves code cells exactly, which is essential for Triton kernel code).
   - URLs → WebFetch.
   - `.md`/`.txt`/`.html` → Read directly.
3. **Read extracted `.txt` / `.md` files** — never the raw PDFs / notebooks.
4. **Source Content Mapping (MANDATORY for multi-file sources)**:
   ...
   - NEVER assume content from filename — `flash_attention.pdf` may be FlashAttention-1, -2, or -3.
...
2. Decide reorg target:
   - **If sources are official Triton docs / tutorials, the Tillet et al. MAPL 2019 Triton paper, FlashAttention-1/2/3 papers, PyTorch 2.x Inductor design docs, OpenAI/Meta blog posts on Triton**: **reorganize into the canonical 6-topic StudyVault structure**. This is the canonical taxonomy for this skill.
   - **If sources are unrelated GPU-adjacent docs** (academic papers on tile compilers, GPU architecture surveys): preserve their natural hierarchy.
...
> **Equal Depth Rule**: even briefly-mentioned subtopics get a full dedicated note supplemented with textbook-level Triton knowledge.

5. **Source-to-note cross-verification** (MANDATORY): record source file + page range per topic.

## Phase D3: Tag Standard

Same registry as Curriculum mode (see [curriculum-workflow.md](curriculum-workflow.md) Phase TU3). When reorganizing into the 6-topic structure, reuse the same `#topic-*` tags.
...
- ASCII diagrams or code snippets for kernel-shape and IR-pipeline concepts.
- Triton identifiers verbatim in English.
...
- Distractors drawn from the realistic Triton confusion pools in `triton-tutor/references/quiz-rules.md`.
