...
> Read this file when updating the dashboard or a concept file. Provides the precise formulas the Triton quiz tutor must implement deterministically.
...
| Concept file does not yet exist | Create it from the template in `triton-tutor/SKILL.md` before adding the first row. |
...
| Dashboard does not yet exist | Create from the template in `triton-tutor/SKILL.md`; initialize all 6 rows with `0/0/-/⬜`. |
